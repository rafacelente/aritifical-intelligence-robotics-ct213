...
from state_machine import GoBackState
...
class RoombaBehaviorTree(BehaviorTree):
    ...
    def __init__(self):
        ...
        left = SequenceNode("left")
        right = SequenceNode("right")

        move_forward = MoveForwardNode()
        move_spiral = MoveInSpiralNode()
        go_back = GoBackNode()
        rotate = RotateNode()

        self.root.add_child(left)
        self.root.add_child(right)
        left.add_child(move_forward)
        left.add_child(move_spiral)
        right.add_child(go_back)
        right.add_child(rotate)
...
class MoveForwardNode(LeafNode):
    ...
    def execute(self, agent):
        self.count += 1
        if (self.count*SAMPLE_TIME >= MOVE_FORWARD_TIME):
            return ExecutionStatus.SUCCESS
...
class RotateNode(LeafNode):
    ...
    def enter(self, agent):
        self.count = 1
        self.rotate_time = random.uniform(math.pi/(2*ANGULAR_SPEED), 3*math.pi/(2*ANGULAR_SPEED))
        agent.set_velocity(0, ANGULAR_SPEED)

    def execute(self, agent):
        self.count += 1
        if self.rotate_time <= self.count*SAMPLE_TIME:
            return ExecutionStatus.SUCCESS
        elif (agent.get_bumper_state()):
            agent.set_bumper_state(False) # Set bumper state to false
            return ExecutionStatus.FAILURE
